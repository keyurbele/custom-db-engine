...
# ==========================================
# 🌲 DATA STRUCTURE: CUSTOM B+ TREE INDEX
# ==========================================
class BPlusNode:
    def __init__(self, is_leaf=False):
        self.is_leaf = is_leaf
        self.keys = []
        self.values = []  # If leaf: stores disk offsets. If internal: stores child BPlusNodes.
        self.next = None  # Pointer to the next leaf node for fast range scans
...
class BPlusTreeIndex:
    ...
    def search(self, key):
        """Traverses the tree to find the exact disk offset for a key."""
        current = self.root
    ...
    def insert(self, key, offset):
        """Inserts a key-offset mapping and handles splitting nodes up to the root."""
        root = self.root
    ...
    def delete(self, key):
        """Simple memory-index key removal for leaf tracking."""
        current = self.root
        while not current.is_leaf:
            i = 0
            while i < len(current.keys) and key >= current.keys[i]:
    ...
    def range_search(self, start_key, end_key):
        """NEW FEATURE: Performs high-efficiency range scans across linked leaf nodes."""
        current = self.root
        while not current.is_leaf:
            i = 0
    ...
    def _insert_non_full(self, node, key, offset):
        if node.is_leaf:
            ...
            if i < len(node.keys) and node.keys[i] == key:
                node.values[i] = offset  # Overwrite matching key offset
            else:
...
# ==========================================
# 🗄️ UPGRADED PAGE-BASED STORAGE ENGINE
# ==========================================
class SimpleStorageEngine:
    def __init__(self, db_filename="data.db", wal_filename="wal.log", cache_capacity=3):
        ...
        # Swapping out the simple dictionary for our custom B+ Tree!
    ...
    def get_range(self, start_key: str, end_key: str):
        """NEW ENGINE ENDPOINT: Uses B+ Tree sequence pointers to fetch range matches."""
        with self.lock:
            ...
            for key, offset in matched_offsets:
                header = self.mm[offset : offset + 7]
                record_type, key_len, value_len = struct.unpack('>BHI', header)
                if record_type != 1:  # Skip tombstones
                    val_start = offset + 7 + key_len
...
if __name__ == "__main__":
    ...
    print("--- 🌲 Testing Custom B+ Tree Range Queries ---")
    db.put("user_10", "Alice")
    db.put("user_45", "Bob")
    db.put("user_25", "Charlie")
    db.put("user_80", "David")
    
    # Run a structural sorted range query scan!
    scan_results = db.get_range("user_20", "user_50")
    print(f"Range Scan Results (user_20 to user_50): {scan_results}")
